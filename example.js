  Rally.onReady(function() {
      Ext.define("CustomApp", {
          ...
          launch: function() {
              this.modelNames = "hierarchicalrequirement", Ext.create("Rally.data.wsapi.TreeStoreBuilder").build({
                  ...
                  success: this._showRenderedGrid,
                  scope: this
              })
          },
          _getSomeStories: function() {
              this.render_grid && this.render_grid.destroy(), this.line_grid && this.line_grid.destroy(), Ext.create("Rally.data.WsapiDataStore", {
                  autoLoad: !0,
          ...
          _revisionRenderer: function(value, meta, record) {
              ...
              return formatted_value_array ? formatted_value_array : (Rally.data.ModelFactory.getModel({
          ...
          _showRenderedGrid: function(store) {
              this.render_grid && this.render_grid.destroy();
                      ...
                      menuItems: [{
                          ...
                          handler: function() {
                              var grid = this.down("rallygridboard").getGridOrBoard(),
                                  cols = grid.columns,
                                  csv = "Formatted ID,Name,Revisions\n";
                              csv += _.map(grid.store.tree.root.childNodes, function(node) {
                                  var revs = node.get("RevisionArray") || "";
                                  return revs = revs.replace(/,/g, "|").replace(/<br\/>/g, "\n,,"), [node.get("FormattedID"), node.get("Name"), revs].join(",")
          ...
          _showLineByLineGrid: function(store) {
              this.line_grid && this.line_grid.destroy(), this.line_grid = Ext.create("Rally.ui.grid.Grid", {
                  store: store,
                  columnCfgs: [{
                      text: "ID",
                      dataIndex: "FormattedID"
